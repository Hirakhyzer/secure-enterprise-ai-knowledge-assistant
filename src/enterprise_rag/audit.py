"""Audit trail for private RAG queries and policy decisions."""
...
def append_audit_record(path: str | Path, record: dict[str, Any]) -> dict[str, Any]:
    """Append a hash-linked audit record for one query or experiment."""
...
def verify_audit_log(path: str | Path) -> dict[str, Any]:
    """Verify hash-chain continuity for a JSONL audit log."""
    destination = Path(path)
    previous_hash = None
    count = 0
